...
async function remove(source,key,row){if(busy)return;const name=label(row);if(!confirm(`Delete “${name}”?\nReference: ${recordId(row)}\nThis removes this record for everyone with access. Attached files may be permanently removed. An audit record is kept.`))return;busy=true;try{const result=await api().deleteManagedRecord(source,key,row);app().showToast(result.warning||'Record deleted.');if(dialog.open)await load();if(source==='database'){if(key.startsWith('training_')||['learners','learner_attendance','cohort_messages'].includes(key))window.CAGE_ACADEMY?.load();if(key.startsWith('academy_'))window.CAGE_ADMISSIONS?.reload();if(['job_openings','job_applications','interviews','employee_documents'].includes(key))window.CAGE_HR_UI?.load();}if(typeof renderProjectWorkspace==='function'&&typeof activeProjectId!=='undefined'&&activeProjectId)renderProjectWorkspace();return true;}catch(e){app().showToast(e.message);if(dialog.open)status.textContent=e.message;return false;}finally{busy=false;if(dialog.open)draw();}}
...
window.addEventListener('cage:access-closed',()=>{generation++;records=[];dialog.close();list.replaceChildren();pages.replaceChildren();});
window.addEventListener('cage:session-ready',()=>{generation++;records=[];dialog.close();list.replaceChildren();pages.replaceChildren();});
...
const filters={quotes:{q:'',client:'',year:'',status:'',page:1},invoices:{q:'',client:'',year:'',status:'',page:1}};

...
panel('mywork',`<div class="section-intro"><div><p class="date-line">Your priorities</p><h2>My work</h2><p>Assignments, equipment and plans that belong to you.</p></div><button class="primary-button" id="add-personal-reminder">＋ Set reminder</button></div><p id="personal-error" role="status"></p><div class="personal-columns"><article class="surface"><h3>My reminders</h3><div id="personal-reminders"></div></article><article class="surface"><h3>My responsibilities</h3><div id="personal-responsibilities"></div></article></div>`);
...
function renderMine(){
 ...
 add('tasks','tasks','Task',r=>r.owner===me()&&!['Done','Completed'].includes(r.status),r=>r.title);
 ...
 $('personal-responsibilities').innerHTML=mine.map(r=>`<div class="personal-row"><div><small>${r.label}${r.due?' · '+esc(r.due):''}</small><button class="personal-link" data-personal-view="${r.module}" data-personal-target="${esc(r.id)}">${esc(r.title)}</button></div><button data-plan-title="${esc(r.title)}">Remind me</button></div>`).join('')||'<p>No current assignments found for your account.</p>';
}
...
window.CAGE_PERSONAL={applyAccess,updateBadge,refreshView,view(v){if(v!=='chat'&&recorder?.state==='recording')recorder.stop();if(v==='mywork'||v==='notifications'){refreshView();refresh();}if(v==='access')renderAccess();}};

...
function cageConfirm({ title = "Confirm action", message = "Are you sure?", confirmText = "Confirm", kicker = "Please confirm", danger = false } = {}) {
  const dialog = document.getElementById("confirm-dialog");
  const form = document.getElementById("confirm-form");
  const action = document.getElementById("confirm-action-button");
  document.getElementById("confirm-dialog-title").textContent = title;
  document.getElementById("confirm-dialog-message").textContent = message;
  document.getElementById("confirm-dialog-kicker").textContent = kicker;
...
function renderDashboard() {
  ...
  document.getElementById("chat-nav-count").textContent = state.messages.filter(chatMessageIsUnread).length;
...
function chatReadStorageKey() {
  const member = window.CAGE_BACKEND?.currentMemberId?.() || "alexander";
  return `cage-chat-read-v1-${member}`;
}

function chatReadState() {
  try { return JSON.parse(localStorage.getItem(chatReadStorageKey()) || "{}"); } catch { return {}; }
}

function markChatThreadRead(threadId) {
  const messages = messagesForThread(threadId);
  const last = messages.at(-1);
  if (!last) return;
  const read = chatReadState();
  read[threadId] = last.id;
  localStorage.setItem(chatReadStorageKey(), JSON.stringify(read));
}

function chatMessageIsUnread(message) {
  const currentMember = window.CAGE_BACKEND?.currentMemberId?.() || "alexander";
  if (!message || message.sender === currentMember || !message.sender) return false;
  const threadId = threadIdForMessage(message);
  const marker = chatReadState()[threadId];
  const messages = messagesForThread(threadId);
  if (!marker) return message.unread === true;
  const markerIndex = messages.findIndex(item => item.id === marker);
  const messageIndex = messages.findIndex(item => item.id === message.id);
  if (markerIndex < 0) return message.unread === true;
...
function renderChat() {
  ...
  document.getElementById("chat-channel-list").innerHTML = threads.length ? threads.map(thread => {
    ...
    const last = messages.at(-1);
    const currentMember = window.CAGE_BACKEND?.currentMemberId?.() || "alexander";
    const unread = messages.filter(chatMessageIsUnread).length;
...
async function handleRequestAction(requestId, action) {
  ...
  if (action === "lose") {
    const confirmed = await cageConfirm({
      kicker: "Request outcome",
      title: "Close as lost or declined?",
      message: `${request.number} will be moved to Lost / Declined. The request history will remain available for reporting and future follow-up.`,
      confirmText: "Close request",
      danger: true
    });
    if (confirmed) changeRequestStage(request.id, "Lost / Declined");
  }
}
...
document.getElementById("import-workspace").addEventListener("change", async event => {
  ...
  try {
    ...
    if (!await cageConfirm({
      kicker: "Workspace recovery",
      title: "Import this backup?",
      message: "This will replace the shared workspace with the selected CAGE backup. The action is significant and should only be used with a verified backup file.",
...
document.getElementById("reset-prototype").addEventListener("click", async () => {
  if (!await cageConfirm({
    kicker: "Administrator action",
    title: "Restore sample data?",
    message: "This replaces the shared workspace records with the original sample data. Export a backup first if any current workspace information must be retained.",
